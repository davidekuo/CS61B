...
public class ArrayHeap<T> {
    ...
    public int changePriority(T item, double priority) {
        int index = -1;
        ...
        if (index != -1) {
            double prevPriority =  contents[index].myPriority;
            contents[index] = new ArrayHeap.Node(item, priority);
    ...
    private void resize(int capacity) {
        ...
        if (this.contents.length - 1 >= 0) {
            System.arraycopy(this.contents, 1, temp, 1, this.contents.length - 1);
        }
        this.contents = temp;
    }
